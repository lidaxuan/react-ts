 ...
 * @LastEditTime: 2021-01-22 18:01:44
 * @FilePath: /react-ts-antvg6/src/command/formatNodeXYCopyCopyCopy.js
 */
/**
 * x----300
 * y---- 50
...
export default class FormatNode {
  ...
  start() {
    ...
    let orderNodes = db.select({ nodeType: 'ORDER' });

    //遍历数组，默认arr中的某一个元素为最大值，进行逐一比较
    // var min;
    // for (var i = 0; i < orderNodes.length; i++) {
    //   for (var j = i; j < orderNodes.length; j++) {
    //     if (orderNodes[i].x > orderNodes[j].x) {
    //       min = orderNodes[j];
    //       orderNodes[j] = orderNodes[i];
    //       orderNodes[i] = min;
    //     }
    //   }
    // }
    ...
    for (let i = levelNodeArr.length - 1; i >= 0; i--) {
      if (i >= 1) {
        const pid = levelNodeArr[i].map(item => {
          return item.pid;
        });
