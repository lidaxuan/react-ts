...
import { judgRelationNodeType } from './enum';
...
class ProcessingData {
  ...
  save(callBack){
    ...
    this.methodCall((params) => {
      ...
      if (_.isArray(params)) {
        params = _.pull(_.compact(params)); // 去重 去空
      }
      console.timeEnd();
  ...
  methodCall (callBack) {
    ...
    // end     ========================
    let formatData = this.formatingData(params); // 将格式化好的数据进行节点判断
    this.judgRelation(formatData.treeDetail, (result) => {
      formatData.treeDetail = _.cloneDeep(result);

      const res = this.checkItme(formatData); // 检查成功是 true 否则是 id
      if (res !== true) { // 节点 id 数组
        return callBack(res);
      }
      return callBack(formatData);
    });
  }
  // 格式化 顶部基本 数据
  formatBaseInfo(data) {
  ...
  judgRelation(data, callBack) {
    ...
    if (judgNodes && judgNodes.length) {
      let relationId = [];
      judgNodes.forEach(item => {
        relationId = relationId.concat(safeGet(item, 'data.relation'));
      });
      relationId = [...new Set(_.compact(relationId))];
      for (let i = 0; i < relationId.length; i++) {
        if (orderIds.indexOf(relationId[i]) === -1) {
          flag = false;
        }
      }

      if (!flag) {
        this.spliceRelation(judgNodes, orderIds, (res) => {
  ...
  spliceRelation(judgNodes, orderIds, callBack) {
    MessageBox.confirm('节点中存在脏数据,是否删除', '提示', {
      ...
      judgNodes.forEach((item, index) => {
        ...
        while (i--) {
          if (orderIds.indexOf(relation[i]) === -1) {
  ...
  checkNode(nodes = []) {
    ...
    // console.log('medidaList', medidaList);
    ...
    if (nodeTypeArr.indexOf('MEDIA') === -1) {
      if (nodeTypeArr.indexOf('ORDER') !== -1) {
  ...
  formatingData(data) {
    ...
    for (let i = 0; i < budgetForkNode.length; i++) { // 修改数据
      for (let k = 0, len = budgetForkNode[i].data.value; k < len.length; k++) {
